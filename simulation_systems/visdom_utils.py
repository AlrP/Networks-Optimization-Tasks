...
class VisdomLinePlotter:
    ...
    def __init__(self, env_name: str = 'simulation_project', viz: bool = False, connection: str = 'default'):
        ...
        if not viz:
            ...
            if self.connection == 'default':
                ...
                self.viz = Visdom(server=self.connection, env=self.env)
        else:
            self.viz = viz
    ...
    def create_plots(self, n_plots):
        ...
        for plot_name, xlabel, ylabel, title, legend in zip(
                ...
                ['Claims Simulation', 'Done Claims', 'Missed Claims', 'Resources', 'Agent Model', 'Customer Models',
                 'Revenue'],
    ...
    def update_visdom_plots(self, claims_aggregator, customer_list, price=1):
        ...
        while not claims_aggregator.stop_simulation:
            ...
            dicts = ((claims_dict, 'claims_plot'),
                     (claims_done_dict, 'done_claims_plot'),
                     (claims_loss_dict, 'missed_claims_plot'),
                     (claims_done_dict, 'revenue_plot'),
                     (customer_types_dict, 'customer_models')
                     )

            for d, d_name in dicts:
                for k, v in d.items():
                    self.update_plot([t], [v], f'Node {k}', d_name)
    ...
    def start(self, claims_aggregator, customer_list):
        """
        Используем threading для псевдопараллелизации процесса
        """
